module ApplicationHelper
	...
	def show_publish_state
		@state = State.find_by(name: 'show')
	end

	# FIX should just get the 'show' publishes
	...
	def total_publishes(user)  
        Publish.where("publishes.state_id = ? AND publishes.user_id = ?", 
				show_publish_state.id, user.id).order("publishes.created_at asc")
	end
