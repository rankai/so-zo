class PublishesController < ApplicationController
	...
	def hot
		...
		if params[:publish_category] 
			...
			if @category.nil?			
                ...
                @hot_publishes = @hot_publishes.where("publishes.publish_category_id = ? AND publishes.state_id = ?", 
	...
	def index
		...
		if params[:publish_category]
			...
			@publishes = Publish.joins(products: :product_images).where('publishes.publish_category_id LIKE ? 
				AND publishes.name LIKE ? AND publishes.state_id = ?', category_id, keywords, 
				state.id).order("publishes.created_at desc").page(params[:page])

			# p "publishes quantity: #{@publishes.count}"

		else	

			@publishes = Publish.where("publishes.state_id = ?", 
				state.id).order("publishes.created_at desc").page(params[:page])
	...
	def works
		...
		if params[:user_id] 
			...
			if params[:publish_category] || params[:sell]
				...
				@publishes = Publish.where('publishes.publish_category_id LIKE ? AND publishes.state_id =? AND publishes.user_id = ?', 
				category_id, state.id, @user.id).order("publishes.created_at desc")
				p "publishes quantity: #{@publishes.count}"
				if params[:sell] == '1'
			...
			@publishes =  Publish.where("publishes.state_id = ? AND publishes.user_id = ?", 
				state.id, @user.id).order("publishes.created_at desc")
	...
	def commit
		...
		# clear session
		session[:publish_id] = nil
		product_active_state = State.find_by(name: 'active') # TODO should set show as publishes

		respond_to do |format|
			if Product.where(publish: @publish).update_all(state_id: product_active_state.id)
				# TODO Set publish to show state to active it
				publish_show_state = State.find_by(name: 'show')
				@publish.update_attribute "state_id", publish_show_state.id
